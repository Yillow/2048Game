...
public class Board {
     ...
     **/
    public void swipeLeft() {  // it only moves the gang one towards the left so if theres 2 blanks it will only move once.
        // WRITE YOUR CODE HERE
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                int n = gameBoard[i][j];
                int h = j - 1;
                while(h >= 0 && gameBoard[i][h] == 0){
                    gameBoard[i][h + 1] = gameBoard[i][h];
                    h = h - 1;
                }
                gameBoard[i][h+1] = n;
            }
